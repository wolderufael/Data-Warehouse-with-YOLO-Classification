...
#     def load_channels_from_json(self,file_path):
#         try:
...
#     async def scrape_channel(self,client, channel_username, writer, media_dir, num_messages):
#         try:
...
#                 csv_filename = f"data/{channel[1:]}_data.csv"  # Remove '@' from channel name
#                 with open(csv_filename, 'a', newline='', encoding='utf-8') as file:
#                     writer = csv.writer(file)
#                     writer.writerow(['Channel Title', 'Channel Username', 'ID', 'Message', 'Date', 'Media Path'])
                    
#                     await self.scrape_channel(client, channel, writer, media_dir, num_messages_to_scrape)
...
async def main():
    ...
    try:
        ...
        # Load channels from JSON file
        channels, comments = load_channels_from_json('../data/channels.json')
